...
let
  cfg = config.hardware.raspberry-pi.display-rp1;
in
{
  options.hardware.raspberry-pi.display-rp1 = {
    enable = lib.mkEnableOption "RP1 display PrimaryGPU configuration (RPi5 only)" // {
      default = true;
    };
  };
