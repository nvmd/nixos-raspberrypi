...
, libidn, libtasn1, p11-kit, zlib
, cmake, nasm, pkg-config, gnutls
, vdpauSupport ? true, libvdpau # "${VDPAU_SUPPORT}" = "yes" -a "${DISPLAYSERVER}" = "x11"
, dav1dSupport ? true, dav1d # target_has_feature "(neon|sse)"
, v4l2Support ? true, libdrm # "${V4L2_SUPPORT}" = "yes"
, v4l2RequestSupport ? true, systemd, udev # raspberryPi == true && raspberryPiDevice = "RPi4" –> "${PKG_V4L2_REQUEST}" = "yes"
, vaapiSupport ? true, libva # "${VAAPI_SUPPORT}" = "yes"
...
let
  ...
  myConfigureFlags = [
    ...
    "--disable-mmal"
    "--enable-neon"
  ] ++ lib.optionals raspberryPi [ #"--disable-rpi"
                                   "--enable-sand"
                                   ]
    ++ lib.optionals v4l2Support [ "--enable-v4l2_m2m"
                                   "--enable-libdrm"
  ...
  libreelecConfigureFlags = [
    ...
    ++ lib.optionals v4l2RequestSupport [
      "--enable-libudev"
      "--enable-v4l2-request"
    ] ++ lib.optionals (raspberryPi && (raspberryPiVersion == "4")) [ #PKG_FFMPEG_HWACCEL
